...
export default function Movie(props) {
  ...
  return (
    <div className="container w-50">
      ...
      {movies.map((item) => (
        <li key={item.id} className="list-group-item">{item.name}<button className="btn btn-outline-secondary float-right" onClick={() => handleDeleteClick(item.id)}>X</button>  </li>
      ))}
    </ul>
    </div>
  );
